    ...
    while not open_list.empty():
        ...
        for neighbor in neighbors_list:
            if neighbor in closed_list:
                continue
            given_cost = current_node[1] + distance(current_node, neighbor)
            neighbor_coord = neighbor[2]

            if neighbor not in open_list:
                final_cost = given_cost + distance(neighbor, finish)
...
def main(argv):
    ...
    print a_star(start, finish, grid)
